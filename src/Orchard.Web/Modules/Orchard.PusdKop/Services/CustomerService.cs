...
namespace Orchard.PusdKop.Services
{
    public class CustomerService : ICustomerService
    {
        ...
        public QuestionnairePart GetQuestionnaire(int customerId, string questionnaireType)
        {
            return _orchardServices.ContentManager.Query<QuestionnairePart, QuestionnairePartRecord>()
                .Where(x => x.CustomerId == customerId && x.Type == questionnaireType).List().FirstOrDefault();
        }

        public QuestionnairePart CreateQuestionnaire(int customerId, string questionnaireType)
        {
            return _orchardServices.ContentManager.Create<QuestionnairePart>("Questionnaire", x =>
            {
                x.Type = questionnaireType;
